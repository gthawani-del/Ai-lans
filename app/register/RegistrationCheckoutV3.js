...
import {getPhoneCountry,normalizeNationalPhone,phoneCountries} from '../../lib/forms/phone-country';
...
function SmartPhoneField({values,setValue,register,error}){
  const [open,setOpen]=useState(false);
  const [query,setQuery]=useState('');
  const wrapRef=useRef(null);
  const selected=getPhoneCountry(values.country);
  const filtered=phoneCountries.filter(item=>`${item.name} ${item.dial}`.toLowerCase().includes(query.toLowerCase())).slice(0,80);

  useEffect(()=>{
    function outside(event){if(wrapRef.current&&!wrapRef.current.contains(event.target))setOpen(false);}
  ...
  function choose(meta){
    setValue('country',meta.name,{shouldDirty:true,shouldTouch:true,shouldValidate:true});
    setValue('phone_country_code',meta.dial,{shouldDirty:true,shouldTouch:true,shouldValidate:true});
    setValue('phone',normalizeNationalPhone(values.phone,meta.name),{shouldDirty:true,shouldTouch:true,shouldValidate:true});
    setOpen(false);
    setQuery('');
  }

  function changePhone(event){
    const clean=normalizeNationalPhone(event.target.value,values.country);
    setValue('phone',clean,{shouldDirty:true,shouldTouch:true,shouldValidate:true});
  }

  return <div className="field phoneField"><label htmlFor="phone">Phone / WhatsApp *</label><div className="smartPhone" ref={wrapRef}><button type="button" className="countryComboTrigger" aria-haspopup="listbox" aria-expanded={open} onClick={()=>setOpen(v=>!v)}><span className="flagEmoji" aria-hidden="true">{selected.flag}</span><strong>{selected.dial}</strong><span className="comboChevron" aria-hidden="true">⌄</span></button><div className="inputWrap phoneNumber"><span className="fieldIcon"><UiIcon name="phone" size={17}/></span><input id="phone" type="tel" autoComplete="tel-national" inputMode="numeric" pattern="[0-9]*" maxLength={selected.code==='IN'?10:15} value={values.phone||''} placeholder={selected.code==='IN'?'9876543210':'Phone number'} onChange={changePhone}/></div>{open&&<div className="countryComboPopover"><div className="countrySearch"><UiIcon name="search" size={15}/><input autoFocus aria-label="Search country or dial code" value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search country or code"/></div><div className="countryOptions" role="listbox" aria-label="Country calling codes">{filtered.map(item=><button type="button" role="option" aria-selected={item.code===selected.code} className={item.code===selected.code?'countryOption selected':'countryOption'} onClick={()=>choose(item)} key={item.code}><span className="flagEmoji" aria-hidden="true">{item.flag}</span><span>{item.name}</span><strong>{item.dial}</strong>{item.code===selected.code&&<UiIcon name="check" size={14}/>}</button>)}</div></div>}</div><input type="hidden" {...register('phone')}/><input type="hidden" {...register('phone_country_code')}/><input type="hidden" {...register('country')}/><FieldError message={error}/>{selected.code==='IN'&&<small className="phoneHint">10-digit Indian mobile number</small>}</div>;
}
...
export default function RegistrationCheckoutV3(){
  ...
  async function confirmApplication(data){
    ...
    try{
      const response=await fetch(API+'/submit-attendee-registration',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({...data,form_version:'one_page_v3',_website:''})});
  ...
  return <main className="regShell onePageRegistration registrationCheckout" data-theme={theme} data-text-scale={textScale} style={{fontSize:`${textScale}%`}}><div className="regFrame onePageFrame"><aside className="regRail"><a href="/" className="regBrand"><AILabMark/><span>AI LAB<small>AI BUSINESS<br/>TRANSFORMATION</small></span></a><h2>From Ideas to<br/>Real Impact</h2><p>A hands-on workshop for CXOs, founders and business leaders.</p><div className="benefits"><div><i><UiIcon name="people"/></i><span>Build <b>Real AI MVPs</b><small>in two days</small></span></div><div><i><UiIcon name="bulb"/></i><span>Learn from<br/>experts & peers</span></div><div><i><UiIcon name="network"/></i><span>Expand<br/>your network</span></div><div><i><UiIcon name="rocket"/></i><span>Leave with a<br/>90-day action plan</span></div></div><div className="event"><b><UiIcon name="calendar" size={24}/></b><span>25 – 26 Sep 2026<small>Sofitel BKC, Mumbai</small></span></div><blockquote>“Practical, focused and immediately applicable. A must for business leaders.”<small>— Previous Attendee</small></blockquote><div className="railArt"><strong>Build<br/>What’s Next</strong></div></aside>
      ...
      <section className="regMain onePageMain"><div className="moduleTrail moduleTrailFour" aria-label="Registration sections"><span className="active">01 About You</span><span className={unlockedModule>=2?'active':''}>02 AI Readiness</span><span className={unlockedModule>=3?'active':''}>03 What You’ll Build</span><span className={paymentSession?'active':''}>04 Payment</span></div>
        <form className="regCard onePageCard" onSubmit={handleSubmit(confirmApplication,handleInvalid)} noValidate><div className="formHead onePageHead checkoutHead compactCheckoutHead"><div className="checkoutTitleBlock"><h1 className="registrationTitle">ATTENDEE REGISTRATION</h1><p>Complete the sections below, review your details, then continue to secure payment.</p></div><div className="headerUtilities"><DisplayControls theme={theme} onToggleTheme={changeTheme} textScale={textScale} onTextScale={changeTextScale} textOpen={textOpen} setTextOpen={setTextOpen}/><div className="time"><UiIcon name="clock" size={21}/><span>Takes about<br/><b>3 minutes</b></span></div></div></div>

          <section className="formModule profileModule" id="module-about"><ModuleHeader number="01" title="About You" subtitle="Your contact and professional profile." complete={profileReady}/><div className="moduleBody"><div className="two"><div className="field"><label htmlFor="full_name">Full Name *</label><div className="inputWrap"><span className="fieldIcon"><UiIcon name="user" size={17}/></span><input id="full_name" autoComplete="name" maxLength={120} placeholder="Enter your full name" {...register('full_name')}/></div><FieldError message={errors.full_name?.message}/></div><div className="field"><label htmlFor="email">Email Address *</label><div className="inputWrap"><span className="fieldIcon"><UiIcon name="mail" size={17}/></span><input id="email" type="email" autoComplete="email" maxLength={254} placeholder="name@company.com" {...register('email')}/></div><FieldError message={errors.email?.message}/></div></div><div className="phoneCityRow"><SmartPhoneField values={values} setValue={setValue} register={register} error={errors.country?.message||errors.phone_country_code?.message||errors.phone?.message}/><div className="field cityField"><label htmlFor="city">City *</label><div className="inputWrap"><span className="fieldIcon"><UiIcon name="pin" size={17}/></span><input id="city" autoComplete="address-level2" maxLength={120} placeholder="Mumbai" {...register('city')}/></div><FieldError message={errors.city?.message}/></div></div><div className="profileRow profileRowPrimary"><div className="field"><label htmlFor="career_role">Current Role *</label><select id="career_role" {...register('career_role')}><option value="">Select role</option>{roles.map(role=><option key={role} value={role}>{role}</option>)}</select><FieldError message={errors.career_role?.message}/></div><div className="field"><label htmlFor="company">Company / Organisation *</label><div className="inputWrap"><span className="fieldIcon"><UiIcon name="building" size={17}/></span><input id="company" autoComplete="organization" maxLength={160} placeholder="Enter company name" {...register('company_organisation')}/></div><FieldError message={errors.company_organisation?.message}/></div><div className="field"><label htmlFor="industry">Industry *</label><select id="industry" {...register('industry')}><option value="">Select industry</option>{industries.map(item=><option key={item} value={item}>{item}</option>)}</select><FieldError message={errors.industry?.message}/></div></div><div className="profileRow profileRowSecondary"><div className="field"><label htmlFor="experience">Experience *</label><select id="experience" {...register('years_experience')}><option value="">Select range</option>{experienceRanges.map(item=><option key={item} value={item}>{item}</option>)}</select><FieldError message={errors.years_experience?.message}/></div><div className="field"><label htmlFor="function">Primary Function *</label><select id="function" {...register('functional_area')}><option value="">Select function</option>{functionalAreas.map(item=><option key={item} value={item}>{item}</option>)}</select><FieldError message={errors.functional_area?.message}/></div></div></div></section>

          <section className={`formModule readinessModule ${unlockedModule<2?'isLocked':''}`} id="module-readiness"><ModuleHeader number="02" title="Your AI Readiness" subtitle="A quick snapshot of where you are today." locked={unlockedModule<2} complete={unlockedModule>=2&&readinessReady}/>{unlockedModule>=2&&<div className="moduleBody revealBody"><fieldset className="aiLevelField"><legend>AI Experience *</legend><div className="aiLevelGroup">{aiLevels.map(level=><button type="button" className={values.ai_experience_level===level?'on':''} aria-pressed={values.ai_experience_level===level} onClick={()=>setValue('ai_experience_level',level,{shouldDirty:true,shouldTouch:true,shouldValidate:true})} key={level}>{level}</button>)}</div><FieldError message={errors.ai_experience_level?.message}/></fieldset><div className="toolField"><label htmlFor="toolPicker">AI tools you use <span>Optional</span></label><div className="toolPickerRow"><div className="selectedTools" aria-live="polite">{currentTools.length?currentTools.map(tool=><button type="button" className="toolChip" onClick={()=>removeTool(tool)} aria-label={`Remove ${tool}`} key={tool}>{tool}<span>×</span></button>):<span className="emptyTools">No tools selected yet</span>}</div><select id="toolPicker" value="" onChange={addTool}><option value="">+ Add AI tool</option>{aiTools.filter(tool=>!currentTools.includes(tool)).map(tool=><option key={tool} value={tool}>{tool}</option>)}</select></div><FieldError message={errors.ai_tools?.message}/></div>{currentTools.includes('Other')&&<div className="field compactReveal"><label htmlFor="other_ai_tool">Other AI tool *</label><input id="other_ai_tool" maxLength={120} placeholder="Enter the tool name" {...register('other_ai_tool')}/><FieldError message={errors.other_ai_tool?.message}/></div>}</div>}</section>
